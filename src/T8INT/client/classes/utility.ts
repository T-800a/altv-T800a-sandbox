...
export class Raycast {
   ...
   static line(radius, distance) {
      ...
      if (DEBUG){ if (rcInterval != null) alt.clearInterval(rcInterval); };
      ...
      if (DEBUG){ rcInterval = alt.setInterval(() => { natives.drawLine(position.x, position.y, position.z, farAway.x, farAway.y, farAway.z, 255, 0, 0, 255); }, 0); };
   ...
   static result(ray) {
      ...
      let _id = 0;
      if( alt.Entity.getByScriptID(hitEntity) ) { _id = alt.Entity.getByScriptID(hitEntity).id; }
